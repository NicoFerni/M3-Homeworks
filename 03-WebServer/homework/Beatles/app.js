...
http
  .createServer(function (req, res) {
    ...
    if (req.url.substring(0, 5) === "/api/") {
      ...
      const beatleFounded = beatles.find((b) => encodeURI(b.name) === beatle);
      if (beatleFounded) {
        res.writeHead(200, { "Content-Type": "application/json" });
        return res.end(JSON.stringify(beatleFounded));
      } else {
      ...
      if (beatleFounded) {
        fs.readFile("./beatle.html", "utf-8", function (err, data) {
          if (err) {
            ...
            data = data.replace('{name}', beatleFounded.name)
            data = data.replace('{birthdate}', beatleFounded.birthdate) 
            data = data.replace('{profilepic}', beatleFounded.profilePic)
            res.writeHead(200, { "Content-Type": "text/html" });
            return res.end(data);
          }
        });
